...
async function initializeDatabase(retries = 5, delay = 3000) {
  ...
  try {
    ...
    await client.query(`
      ...
      -- My library: Mashes a host keeps to host again. Owner-private.
      CREATE TABLE IF NOT EXISTS saved_mashes (
      ...
      CREATE INDEX IF NOT EXISTS saved_mashes_owner_updated_idx
        ON saved_mashes (owner_id, updated_at DESC);
...
const dbApi = {
  ...
  // --- My library (saved_mashes) ---
  // Every statement filters on owner_id in SQL, so an id that belongs to
  // another host behaves exactly like an id that does not exist.

  async listSavedMashes(ownerId) {
    const result = await pool.query(
      `SELECT id, name, kind, question_count, created_at, updated_at
       FROM saved_mashes
       WHERE owner_id = $1
  ...
  async countSavedMashes(ownerId) {
    const result = await pool.query(
      'SELECT COUNT(*)::int AS count FROM saved_mashes WHERE owner_id = $1',
      [ownerId]
    );
    return result.rows[0].count;
  },

  async getSavedMash(id, ownerId) {
    const result = await pool.query(
      `SELECT id, name, kind, markdown, question_count, created_at, updated_at
       FROM saved_mashes
  ...
  async createSavedMash({ ownerId, kind, name, markdown, questionCount }) {
    const result = await pool.query(
      `INSERT INTO saved_mashes (owner_id, kind, name, markdown, question_count)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, name, kind, question_count, created_at, updated_at`,
      [ownerId, kind, name, markdown, questionCount]
    );
    return result.rows[0];
  },

  async updateSavedMash(id, ownerId, { name, markdown, questionCount }) {
    const result = await pool.query(
      `UPDATE saved_mashes
       SET name = $3, markdown = $4, question_count = $5, updated_at = NOW()
       WHERE id = $1 AND owner_id = $2
       RETURNING id, name, kind, question_count, created_at, updated_at`,
      [id, ownerId, name, markdown, questionCount]
    );
    return result.rows[0] || null;
  },

  async deleteSavedMash(id, ownerId) {
    const result = await pool.query(
      'DELETE FROM saved_mashes WHERE id = $1 AND owner_id = $2',
      [id, ownerId]
    );
